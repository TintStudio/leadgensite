  ...
  try {
    ...
    if (credError || !credentials) {
      return NextResponse.json(
        { error: credError || "No AI API key found. Please configure it in Settings." },
        { status: 400 }
      );
    }
    ...
    if (websiteError || !website || !website.projects) {
      return NextResponse.json(
        { error: "Website or associated project not found" },
        { status: 404 }
      );
    }
    ...
    // 3. Mark website as generating
    await supabase
      .from("websites")
      .update({ status: "generating" } as never)
      .eq("id", websiteId);

    // 4. Fetch Services and Service Areas
    ...
    const targetKeyword = (brandSettings.target_keyword as string) || `${project.niche} in ${project.city}`;
    ...
    // --- STEP A: Generate Homepage (Section-by-Section Modular Generation) ---
    // Fetch modular prompts from active prompt store (with fallbacks)
    ...
    if (
      ...
      // Execute section generations in parallel for maximum speed and isolated quality
      const [
        seoData,
        heroData,
        aboutData,
      ...
      // Assemble unified homepage content
      homepageContent = {
      ...
      // Fallback: Bulk prompt
      const fallbackPrompt = getPromptTemplateById("homepage_full") || HOMEPAGE_PROMPT_TEMPLATE;
    ...
    if (services && services.length > 0) {
      for (let i = 0; i < services.length; i++) {
        ...
        generatedPages.push({ page_type: "service", title: service.name, slug: `services/${service.slug}` });
      }
    }
    ...
    if (areas && areas.length > 0) {
      for (let i = 0; i < areas.length; i++) {
        ...
        generatedPages.push({ page_type: "service-area", title: area.area_name, slug: `areas/${area.slug}` });
      }
    }
    ...
    if (blogTitles && blogTitles.length > 0) {
      for (let i = 0; i < blogTitles.length; i++) {
        ...
        generatedPages.push({ page_type: "blog", title: blogTitle, slug: `blog/${blogContent.slug}` });
      }
    }
    ...
    const contactContent = {
      ...
      emergency_note: "Immediate dispatch available for active emergencies. Call directly for prioritized support.",
    };
    ...
    // 1. Fetch all pages with their validated content from database
    ...
    if (allPages && allPages.length > 0) {
      const templateSlug = (website.brand_settings?.template_id as string) || "plumber-pro";
      ...
      // Upload all HTML pages + sitemap + robots
    ...
    // Mark website as generated with storage_path
    await supabase
      .from("websites")
      .update({
        status: "generated",
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to generate website content" },
      { status: 500 }
    );
  }
}
